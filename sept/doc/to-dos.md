...
-   Make sure that egui events that don't actually alter anything don't cause a re-render.
-   See if it's possible to implement editing of UnicodeCharTerm more directly using ApplyEditT on its views, so that edits get applied to the view itself, and the event handling logic gets much simpler.  For example, say the char was `\t` and a key input `n` when the view is `sept::qv::UnicodeCharTermEscCElemView` with `char_index == 1` would alter the `t` to be `n`.  If `char_index == 0` and the input was `n`, then this would alter the whole char to `n` (and this would imply that if the cursor was pointing at the EscCElemView, then the cursor would have to change, and it's not clear how that should work).  Perhaps the ApplyEditT gets access to the cursor as well.  This sort of thing is necessary in order for editing of many data types (e.g. ints, floats) to be tractable.
