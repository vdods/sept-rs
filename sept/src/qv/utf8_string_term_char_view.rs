...
impl<'a> Utf8StringTermCharView<'a> {
    ...
    pub fn new_with_cached_char_count(string: &'a str, char_count: usize) -> Result<Self> {
        assert_eq!(
            string.chars().count(),
            char_count,
            "programmer error: actual char_count does not match specified char_count"
        );
        Ok(Self { string, char_count })
    }
}
