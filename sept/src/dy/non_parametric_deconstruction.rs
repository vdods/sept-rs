...
impl TryFrom<dy::Value> for NonParametricDeconstruction {
    ...
    fn try_from(value: dy::Value) -> std::result::Result<Self, Self::Error> {
        Ok(Self(dy::RUNTIME_LA.read().unwrap().non_parametric_term_code(value.as_ref())?))
...
//         Ok(self.0)
        Ok(dy::RUNTIME_LA.read().unwrap().non_parametric_term_from_code(self.0)?)
    }
