...
pub struct StructTermTerm {
    ...
    /// table (probably the global symbol table), and r#type would be a GlobalSymRefTerm.
        ...
        match type_maybe_dereferenced {
            dy::MaybeDereferencedValue::NonRef(type_value_guts) => {
                match type_value_guts.downcast_ref::<dy::StructTerm>() {
                    ...
                    None => {
                        anyhow::bail!("can't construct a StructTermTerm with type that isn't a StructTerm; type resolved to {}", dy::RUNTIME_LA.read().unwrap().label_of(type_value_guts.type_id()));
                    }
                }
            }
            dy::MaybeDereferencedValue::Ref(type_value_la) => {
                ...
                match type_value_g.downcast_ref::<dy::StructTerm>() {
                    ...
                    None => {
                        anyhow::bail!("can't construct a StructTermTerm with type that isn't a StructTerm; type resolved to {}", dy::RUNTIME_LA.read().unwrap().label_of(type_value_g.type_id()));
                    }
                }
            }
        };
