...
impl dy::Constructor for GlobalSymRefTerm {
    ...
    fn deserialize_parameters_and_construct(
        ...
        // NOTE: This is also probably wrong, since it dereferences this symref during deserialization,
        // and the resolution may not succeed before/during deserialization.  Either need to guarantee
        // that sym refs resolve correctly for deserialization, or create a "deferred construction"
        // term which is executed after the symbols are read in (this latter option doesn't really
        // work for static types, so is probably not tenable generally).
        Ok(self
            .resolved()
            .expect("GlobalSymRefTerm failed to resolve")
            .read()
