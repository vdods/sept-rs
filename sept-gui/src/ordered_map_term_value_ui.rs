        ...
        ui.input_mut(|input_state| {
            if view_ctx.render_address_is_cursor_address() {
                if input_state.consume_key(Modifiers::NONE, Key::Enter)
                    || input_state.consume_key(Modifiers::NONE, Key::K)
                {
                ...
                } else if input_state.consume_key(Modifiers::NONE, Key::V) {
                    ...
                    || input_state.consume_key(Modifiers::NONE, Key::Escape)
                {
                    ...
                    || input_state.consume_key(Modifiers::NONE, Key::PageUp)
                {
                    view_ctx.cursor_address_pop();
                    view_ctx.cursor_address_push(0u32.into());
                    // TODO: use ui.scroll_to_me
                } else if input_state.consume_key(Modifiers::NONE, Key::End)
                    || input_state.consume_key(Modifiers::NONE, Key::PageDown)
                {
                    view_ctx.cursor_address_pop();
                    view_ctx.cursor_address_push(1u32.into());
                    // TODO: use ui.scroll_to_me
                    ...
                    if element_index_delta != 0 {
                        ...
                        if !self.contains_key(&key) {
                            ...
                            let new_key = if element_index_delta < 0 {
                                ...
                                assert!(range_len > 0);
                                let element_abs_delta =
                                    (element_index_delta as u32).min(range_len - 1);
                                let new_key =
                                    range.nth(element_abs_delta as usize).unwrap().0.clone();
                                new_key
                            };
