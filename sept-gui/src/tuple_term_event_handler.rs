...
// NOTE: This is almost identical to the implementation for ArrayTerm.  TODO: Try to de-duplicate somehow.
impl EventHandlerT for sept::dy::TupleTerm {
    fn handle_event(
        &self,
        event: egui::Event,
        ...
        if let Some(cursor_address_token) = cursor_address_token_i.next() {
            ...
            match self.run_single_query(cursor_address_token)? {
                sept::qv::TupleTermQuery::TupleTermElemView(v) => {
            ...
            match event {
                ...
                // TODO: Impl this using egui::Event::Text, since Key::OpenParen doesn't exist
...
impl<'a> EventHandlerT for sept::qv::TupleTermElemView<'a> {
    fn handle_event(
        &self,
        event: egui::Event,
        event_handler_ctx: &mut EventHandlerCtx<'_>,
        ...
        if cursor_address_token_i.peek().is_some() {
            match self.elem_o.as_deref() {
                ...
                None => {
                    anyhow::bail!("TupleTermElemView::handle_event; elem_o is None, can't forward event to it.");
                }
            }
        } else {
            let cursor_len = event_handler_ctx.cursor_address.len() as u32;
            ...
            match event {
                ...
                // TODO: Remove the duplication of this with the below one.
                egui::Event::Key {
                    key: egui::Key::Enter,
                    pressed: true,
                    modifiers,
                ...
                // TODO: Remove the duplication of this with the above one.
                egui::Event::Text(string) if string.starts_with(")") => {
                    // Escape this view by taking off the last cursor token, and then attempting to
                    // advance the cursor using egui::Key::ArrowRight, which is a uniform way to
                    // advance the cursor by one element in all views.
                ...
                egui::Event::Key {
                    ...
                    if self.elem_index < self.tuple_term.len() {
                        let cursor_value = {
